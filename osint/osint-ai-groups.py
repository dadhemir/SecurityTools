        ...
        with open(filename, 'r', encoding='utf-8') as file:
            for line in file:
                ...
                if email and is_email(email):
                    emails.append(email)
                elif email:  # Non-empty line that's not a valid email
...
def create_zip_file(pdf_files, zip_filename):
        ...
        with zipfile.ZipFile(zip_filename, 'w', zipfile.ZIP_DEFLATED) as zipf:
            for pdf_file in pdf_files:
                if os.path.exists(pdf_file):
                    ...
                    print(f"📄 Added to zip: {os.path.basename(pdf_file)}")
                else:
...
def run_batch_osint():
    """Process multiple emails from a txt file and create a zip with all PDF reports"""
    print("\n📁 Batch OSINT Processing")
    print("=" * 50)
    
    # Get input file
    input_file = input("📄 Enter the path to the txt file with emails: ").strip()
    if not input_file:
    ...
    print(f"📧 Found {len(emails)} valid emails to process:")
    for i, email in enumerate(emails, 1):
        print(f"  {i}. {email}")
    
    # Confirm processing
    confirm = input(f"\n🤔 Do you want to process all {len(emails)} emails? (y/n): ").strip().lower()
    ...
    for i, email in enumerate(emails, 1):
        ...
        try:
            # Get OSINT data
            emailrep_data = query_emailrep(email)
            hibp_data = query_hibp(email)
            ...
            print(f"🧠 Generating AI recommendations for {email}...")
            ai_recommendations = analyze_with_openai(ai_prompt)
            
            # Generate PDF report
            print(f"📄 Generating PDF report for {email}...")
            pdf_filename = generate_pdf_report(email, emailrep_data, hibp_data, {}, ai_recommendations, "Email")
    ...
    if pdf_files:
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        zip_filename = f"osint_batch_reports_{timestamp}.zip"
        
        print(f"\n📦 Creating zip file with {len(pdf_files)} reports...")
        if create_zip_file(pdf_files, zip_filename):
            print(f"\n🎉 Batch processing completed!")
            print(f"📊 Successfully processed: {successful_reports}/{len(emails)} emails")
            print(f"📦 Zip file created: {zip_filename}")
            
            # Ask if user wants to clean up individual PDF files
            cleanup = input("\n🗑️  Do you want to delete individual PDF files? (y/n): ").strip().lower()
            if cleanup in ['y', 'yes']:
                for pdf_file in pdf_files:
                    try:
                        os.remove(pdf_file)
                        print(f"🗑️  Deleted: {pdf_file}")
                    except Exception as e:
                        print(f"⚠️  Could not delete {pdf_file}: {str(e)}")
                print("✅ Individual PDF files cleaned up.")
            else:
                print("📄 Individual PDF files kept.")
        else:
            print("❌ Failed to create zip file.")
    else:
        print("❌ No PDF reports were generated.")
...
if __name__ == "__main__":
    ...
    print("=" * 50)
    print("1. Run Single OSINT Analysis")
    print("2. Run Batch OSINT Analysis (from txt file)")
    print("3. Exit")
